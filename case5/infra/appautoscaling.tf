...
resource "aws_appautoscaling_policy" "api_service_alb" {
  ...
  target_tracking_scaling_policy_configuration {
    predefined_metric_specification {
      ...
      resource_label         = "${aws_lb.alb.arn_suffix}/${aws_lb_target_group.ecs_tgs[each.key].arn_suffix}"
    }
...
# SQSワーカー用のスケールアップポリシー
resource "aws_appautoscaling_policy" "sqs_worker_scale_up" {
  for_each           = local.queues
  name               = "${each.key}_sqs-worker-scale-up"
  resource_id        = aws_appautoscaling_target.ecs_services[each.key].resource_id
  scalable_dimension = aws_appautoscaling_target.ecs_services[each.key].scalable_dimension
  service_namespace  = aws_appautoscaling_target.ecs_services[each.key].service_namespace
...
# SQSワーカー用のスケールダウンポリシー
resource "aws_appautoscaling_policy" "sqs_worker_scale_down" {
  for_each           = local.queues
  name               = "${each.key}_sqs-worker-scale-down"
  resource_id        = aws_appautoscaling_target.ecs_services[each.key].resource_id
  scalable_dimension = aws_appautoscaling_target.ecs_services[each.key].scalable_dimension
  service_namespace  = aws_appautoscaling_target.ecs_services[each.key].service_namespace
  policy_type        = "StepScaling"
